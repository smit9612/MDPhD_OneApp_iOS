...
@IBDesignable class AMInputView: UIView,UITextFieldDelegate {
    ...
    @IBInspectable var AcceptableString: String = ""{
      didSet {

        acceptable = AcceptableString
      }
    }
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        guard let lenght = textField.text?.lengthOfBytes(using: .utf8), lenght < 1 else { return }
    ...
    func setViewExpandMode(expand:Bool) {
        ...
        UIView.animate(withDuration: animationDuration) {
            self.layoutIfNeeded()
            self.backWhiteView.alpha = expand ? 0.6 : 0.15
